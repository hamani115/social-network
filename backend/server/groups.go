...
func groupSubroutesHandler(w http.ResponseWriter, r *http.Request) {
	...
	if len(parts) == 2 && parts[1] == "members" {
		...
		listGroupMembersHandler(w, r, groupID)

		return
	}

	if len(parts) == 2 && parts[1] == "invite-candidates" {
		...
		listGroupInviteCandidatesHandler(w, r, groupID)
...
func createGroupHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		errorJSON(w, "could not read group id", http.StatusInternalServerError)
		return
	}
	...
	if err != nil {
		errorJSON(w, "could not add group owner", http.StatusInternalServerError)
		return
	}
...
func listGroupsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if rawLimit := r.URL.Query().Get("limit"); rawLimit != "" {
		...
		if err != nil || parsedLimit <= 0 || parsedLimit > 50 {
			errorJSON(w, "invalid group limit", http.StatusBadRequest)
			return
		}
	...
	if rawOffset := r.URL.Query().Get("offset"); rawOffset != "" {
		...
		if err != nil || parsedOffset < 0 {
			...
			errorJSON(w, "invalid group offset", http.StatusBadRequest)
			return
		}
	...
	rows, err := db.Query(`
			...
			CASE
				...
				WHEN EXISTS (
					...
					WHERE
						...
						AND group_join_requests.requester_id = ?
				...
				WHEN EXISTS (
					...
					WHERE
						...
						AND group_invitations.invitee_id = ?
		...
		WHERE (
			? = ''
			OR LOWER(groups.title) LIKE ?
			OR LOWER(COALESCE(groups.description, '')) LIKE ?
		)
		ORDER BY
		...
		LIMIT ?
		OFFSET ?
	`, currentUserID, currentUserID, currentUserID, currentUserID, searchQuery, searchPattern, searchPattern, limit+1, offset)
...
func getGroupHandler(w http.ResponseWriter, r *http.Request, groupID int) {
	...
	err := db.QueryRow(`
		...
		JOIN users ON users.id = groups.creator_id
		WHERE groups.id = ?
	`, groupID).Scan(&group.ID, &group.CreatorID, &group.CreatorName, &group.Title, &group.Description, &group.CreatedAt)
...
func requestJoinGroupHandler(w http.ResponseWriter, r *http.Request, groupID int) {
	...
	if invitationCount > 0 {
		errorJSON(w, "you already have a pending invitation to this group", http.StatusBadRequest)
		return
	}
...
func acceptGroupJoinRequestHandler(w http.ResponseWriter, r *http.Request, groupID int, requestID int) {
	...
	if err != nil {
		errorJSON(w, "could not verify join request update", http.StatusInternalServerError)
		return
	}

	if rowsAffected == 0 {
		errorJSON(w, "join request is no longer pending", http.StatusConflict)
		return
	}

	err = tx.Commit()
	if err != nil {
		errorJSON(w, "could not accept join request", http.StatusInternalServerError)
		return
	}
	...
	if err != nil {
		log.Printf("group join request %d accepted, but notification failed: %v", requestID, err)
	}
...
func leaveGroupHandler(w http.ResponseWriter, r *http.Request, groupID int) {
	currentUserID := r.Context().Value(userIDKey).(int)
	...
	status, err := getGroupMembershipStatus(currentUserID, groupID)
	...
	if status == "owner" {
		errorJSON(w, "group owner cannot leave the group", http.StatusBadRequest)
		return
	}

	if status != "member" {
		errorJSON(w, "you are not a member of this group", http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		errorJSON(w, "could not remove group event responses", http.StatusInternalServerError)
		return
	}
	...
	if err != nil {
		errorJSON(w, "could not leave group", http.StatusInternalServerError)
		return
	}
	...
	if err != nil {
		errorJSON(w, "could not verify group membership", http.StatusInternalServerError)
		return
	}

	if rowsAffected == 0 {
		errorJSON(w, "group membership no longer exists", http.StatusConflict)
		return
	}
...
func createGroupInvitationHandler(w http.ResponseWriter, r *http.Request, groupID int) {
	...
	if joinRequestCount > 0 {
		errorJSON(w, "user already has a pending request to join this group", http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		log.Printf("group invitation to user %d for group %d created, but notification failed: %v", req.InviteeID, groupID, err)
	}
...
func acceptGroupInvitationHandler(w http.ResponseWriter, r *http.Request, invitationID int) {
	...
	rowsAffected, err := result.RowsAffected()
	if err != nil {
		errorJSON(w, "could not verify invitation update", http.StatusInternalServerError)
		return
	}

	if rowsAffected == 0 {
		errorJSON(w, "invitation is no longer pending", http.StatusConflict)
		return
	}

	err = tx.Commit()
	if err != nil {
		errorJSON(w, "could not accept invitation", http.StatusInternalServerError)
		return
	}
	...
	if err != nil {
		log.Printf("group invitation %d accepted, but notification failed: %v", invitationID, err)
	}
...
func listGroupMembersHandler(w http.ResponseWriter, r *http.Request, groupID int) {
	...
	if rawLimit := r.URL.Query().Get("limit"); rawLimit != "" {
		...
		if err != nil || parsedLimit <= 0 || parsedLimit > 50 {
			errorJSON(w, "invalid member limit", http.StatusBadRequest)
			return
		}
	...
	if rawOffset := r.URL.Query().Get("offset"); rawOffset != "" {
		...
		if err != nil || parsedOffset < 0 {
			...
			errorJSON(w, "invalid member offset", http.StatusBadRequest)
			return
		}
	...
	rows, err := db.Query(`
		...
		WHERE
			...
			AND (? = ''
				...
				OR LOWER(users.first_name || ' ' || users.last_name) LIKE ?
				OR LOWER(COALESCE(users.nickname, '')) LIKE ?
			)
		...
		LIMIT ?
		OFFSET ?
	`, groupID, searchQuery, searchPattern, searchPattern, searchPattern, searchPattern, limit+1, offset)

	if err != nil {
		errorJSON(w, "could not load group members", http.StatusInternalServerError)
		return
	}
	...
	for rows.Next() {
		...
		if err != nil {
			errorJSON(w, "could not read group member", http.StatusInternalServerError)
			return
		}
	...
	if err := rows.Err(); err != nil {
		errorJSON(w, "error while reading group members", http.StatusInternalServerError)
		return
	}
...
func listGroupInviteCandidatesHandler(w http.ResponseWriter, r *http.Request, groupID int) {
	...
	if rawLimit := r.URL.Query().Get("limit"); rawLimit != "" {
		...
		if err != nil || parsedLimit <= 0 || parsedLimit > 50 {
			errorJSON(w, "invalid invite candidate limit", http.StatusBadRequest)
			return
		}
	...
	if rawOffset := r.URL.Query().Get("offset"); rawOffset != "" {
		...
		if err != nil || parsedOffset < 0 {
			errorJSON(w, "invalid invite candidate offset", http.StatusBadRequest)
			return
		}
	...
	rows, err := db.Query(`
		...
		WHERE
			...
			AND NOT EXISTS (
				...
				WHERE
					group_join_requests.group_id = ?
					AND group_join_requests.requester_id = users.id
					AND group_join_requests.status = 'pending'
			)
			...
			AND NOT EXISTS (
				...
				WHERE
					group_invitations.group_id = ?
					AND group_invitations.invitee_id = users.id
					AND group_invitations.status = 'pending'
			)
			AND (
				? = ''
				...
				OR LOWER(users.last_name) LIKE ?
				OR LOWER(users.first_name || ' ' || users.last_name ) LIKE ?
				OR LOWER(COALESCE(users.nickname, '')
				) LIKE ?
			)
		ORDER BY
			LOWER(users.first_name),
		...
		LIMIT ?
		OFFSET ?
	`, currentUserID, groupID, groupID, groupID, searchQuery, searchPattern, searchPattern, searchPattern, searchPattern, limit+1, offset)

	if err != nil {
		errorJSON(w, "could not load invite candidates", http.StatusInternalServerError)
		return
	}

	defer rows.Close()

	users := []GroupInviteCandidate{}
	...
	for rows.Next() {
		...
		if err != nil {
			errorJSON(w, "could not read invite candidate", http.StatusInternalServerError)
			return
		}
	...
	if err := rows.Err(); err != nil {
		errorJSON(w, "error while reading invite candidates", http.StatusInternalServerError)
		return
	}
